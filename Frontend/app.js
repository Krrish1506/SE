...
; (function () {
  ...
  function initHeader() {
    ...
    if (navActions && !$('#authLinkDesktop')) {
        const token = localStorage.getItem('token');
        const loginLink = document.createElement('a');
        loginLink.id = 'authLinkDesktop';
        loginLink.href = 'login.html';
        loginLink.style.marginRight = '1rem';
        loginLink.style.fontWeight = 'bold';
        loginLink.style.color = 'var(--clr-text)';
        loginLink.textContent = token ? 'Account' : 'Login';
  ...
  function initCart() {
    ...
    // Handle Checkout
    const checkoutBtn = $('#cartFlyout .cart-footer .btn.btn-primary');
    if (checkoutBtn) {
      checkoutBtn.addEventListener('click', () => {
        ...
        const token = localStorage.getItem('token');
        if (!token) {
          showToast("Please log in to checkout. Redirecting...", "error");
  ...
  // Fetch products then initialize
  async function boot() {
  ...
  // Export utilities globally for external scripts (like checkout.html)
  window.Cart = Cart;
  window.showToast = typeof showToast !== 'undefined' ? showToast : () => {};
